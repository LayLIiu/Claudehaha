...
import type { ClientMessage, ServerMessage, StreamingFallbackCause, SessionSummary } from './events.js'
...
/**
 * Global channel session ID — mobile clients connect to /ws/global for
 * cross-session events (session_activated, session_status_changed, etc.)
 * rather than subscribing to one specific session.
 */
const GLOBAL_SESSION_ID = '__global__'
const globalClients = new Set<ServerWebSocket<WebSocketData>>()
...
export const handleWebSocket = {
  open(ws: ServerWebSocket<WebSocketData>) {
    ...
    // Global channel — mobile clients subscribe to cross-session events
    if (channel === 'global') {
      globalClients.add(ws)
      console.log(`[WS] Global client connected (${globalClients.size} total)`)
      const msg: ServerMessage = { type: 'connected', sessionId: GLOBAL_SESSION_ID }
      ws.send(JSON.stringify(msg))
      // Immediately send current sessions snapshot
      void sendSessionsSnapshot(ws)
      return
    }
  ...
  message(ws: ServerWebSocket<WebSocketData>, rawMessage: string | Buffer) {
    ...
    // Global channel only accepts ping for keep-alive
    if (ws.data.channel === 'global') {
      try {
  ...
  close(ws: ServerWebSocket<WebSocketData>, code: number, reason: string) {
    ...
    if (channel === 'global') {
      ...
      console.log(`[WS] Global client disconnected (${globalClients.size} remaining)`)
      return
    }
