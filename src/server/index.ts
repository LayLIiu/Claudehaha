...
import { mDNSService } from './services/mDNSService.js'
...
function resolveServerOptions() {
  ...
  const host = readArgValue('--host') || process.env.SERVER_HOST || '0.0.0.0'
...
export function startServer(port = PORT, host = HOST) {
  ...
  // Start mDNS broadcasting for mobile device discovery
  mDNSService.start(serverPort)
