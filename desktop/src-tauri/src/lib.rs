    ...
    let app = builder
        .setup(|app| {
            ...
            if let Some(main_window) = app.get_webview_window("main") {
                // macOS 原生侧栏液态玻璃（NSVisualEffectView）
                use tauri::window::{Color, Effect, EffectState, EffectsBuilder};
                match main_window.set_effects(
                    EffectsBuilder::new()
                        .effect(Effect::Sidebar)
                        .state(EffectState::Active)
                        .build(),
                ) {
                    Ok(_) => eprintln!("[desktop] set_effects(Sidebar) OK"),
                    Err(e) => eprintln!("[desktop] set_effects(Sidebar) failed: {e}"),
                }
                // 让 NSWindow + WebView 都透明，原生毛玻璃透过来
                let _ = main_window.set_background_color(Some(Color(0, 0, 0, 0)));
                // ObjC 直接: 从 NSWindow 找 WKWebView 设 drawsTransparentBackground
                {
                    use objc2::msg_send;
                    let ptr = match main_window.ns_window() {
                        Ok(p) => p as *mut objc2::runtime::AnyObject,
                        Err(_) => {
                            eprintln!("[desktop] failed to get ns_window");
                            std::ptr::null_mut()
                        }
                    };
                    if !ptr.is_null() {
                        unsafe {
                            let cv: *mut objc2::runtime::AnyObject = msg_send![ptr, contentView];
                            if !cv.is_null() {
                                let nsnum = objc2::class!(NSNumber);
                                let nsstr = objc2::class!(NSString);
                                let nscol = objc2::class!(NSColor);
                                let yes: *mut objc2::runtime::AnyObject = msg_send![nsnum, numberWithBool: 1i8];
                                let clr: *mut objc2::runtime::AnyObject = msg_send![nscol, clearColor];
                                let subs: *mut objc2::runtime::AnyObject = msg_send![cv, subviews];
                                let cnt: usize = msg_send![subs, count];
                                for i in 0..cnt {
                                    let v: *mut objc2::runtime::AnyObject = msg_send![subs, objectAtIndex: i];
                                    if v.is_null() { continue; }
                                    let responds: i8 = msg_send![v, respondsToSelector: objc2::sel!(setDrawsTransparentBackground:)];
                                    if responds != 0 {
                                        let k1: *mut objc2::runtime::AnyObject = msg_send![nsstr, stringWithUTF8String: b"drawsTransparentBackground\0".as_ptr() as *const i8];
                                        let _: () = msg_send![v, setValue: yes, forKey: k1];
                                        let k2: *mut objc2::runtime::AnyObject = msg_send![nsstr, stringWithUTF8String: b"underPageBackgroundColor\0".as_ptr() as *const i8];
                                        let _: () = msg_send![v, setValue: clr, forKey: k2];
                                        eprintln!("[desktop] WKWebView transparent (idx={i})");
                                    }
                                }
                            }
                        }
