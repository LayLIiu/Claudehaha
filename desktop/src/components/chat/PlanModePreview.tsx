...
export function PlanPreviewCard({
  ...
  return (
    <div data-testid="plan-preview-card" className="overflow-hidden rounded-lg border border-[var(--color-surface-glass-border)] bg-[var(--color-surface)]">
      <div className="flex items-start gap-2 border-b border-[var(--color-surface-glass-border)] bg-[var(--color-surface-container-low)] px-3 py-2.5">
        <FileText size={15} strokeWidth={2.1} className="mt-0.5 shrink-0 text-[var(--color-brand)]" aria-hidden="true" />
        <div className="min-w-0 flex-1">
          <div className="text-[12px] font-semibold text-[var(--color-token-foreground)]">
            {title}
          </div>
          {filePath ? (
            <div className="mt-0.5 truncate font-[var(--font-mono)] text-[11px] text-[var(--color-token-text-secondary)]">
              {filePath}
            </div>
          ) : null}
        </div>
      ...
      {allowedPrompts.length > 0 && requestedPermissionsTitle ? (
        <div className="border-t border-[var(--color-surface-glass-border)] bg-[var(--color-surface-container-low)] px-3 py-2.5">
          ...
          <div className="space-y-1">
            {allowedPrompts.map((prompt, index) => (
              <div
                ...
                className="rounded-md border border-[var(--color-surface-glass-border)] bg-[var(--color-surface)] px-2.5 py-1.5 text-[11px] text-[var(--color-token-text-secondary)]"
              >
                <span className="font-[var(--font-mono)] font-semibold text-[var(--color-token-foreground)]">
                  {prompt.tool}
                </span>
