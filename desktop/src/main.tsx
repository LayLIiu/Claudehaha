...
export async function bootstrapDesktopApp(
  ...
  try {
    ...
    // Add platform class for macOS Electron (liquid glass support)
    const desktopHost = (window as any).desktopHost
    if (desktopHost?.isDesktop && /Mac/.test(navigator.platform)) {
