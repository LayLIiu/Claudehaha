...
export const useTabStore = create<TabStore>((set, get) => ({
  ...
  openTab: (sessionId, title, type = 'session') => {
    ...
    // When opening settings tab, remember the session we came from
    if (sessionId === SETTINGS_TAB_ID && type === 'settings') {
      // Lazy import to avoid circular dependency
      import('./uiStore').then(({ useUIStore }) => {
        const prevSession = activeTabId && activeTabId !== SETTINGS_TAB_ID ? activeTabId : null
        useUIStore.getState().setSettingsEntrySessionId(prevSession)
      }).catch(() => {})
    }
