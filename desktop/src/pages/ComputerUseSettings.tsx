...
function StatusIcon({ ok }: { ok: boolean | null }) {
  if (ok === null) {
    return <span className="material-symbols-outlined icon-md text-[var(--color-token-text-secondary)]">help</span>
  }
  return ok ? (
    <span className="material-symbols-outlined icon-md text-green-500" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
  ) : (
    <span className="material-symbols-outlined icon-md text-red-400" style={{ fontVariationSettings: "'FILL' 1" }}>cancel</span>
  )
}
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 px-4 rounded-lg bg-[var(--color-surface-container-low)]">
      ...
      <div className="flex-1 min-w-0">
        <span className="text-sm font-medium text-[var(--color-token-foreground)]">{label}</span>
        <span className="ml-2 text-xs text-[var(--color-token-text-secondary)]">{detail}</span>
      </div>
    </div>
  )
}
    ...
    <div className="max-w-2xl space-y-6">
      ...
      <div>
        <div className="flex items-center justify-between gap-4">
          <h2 className="text-lg font-semibold text-[var(--color-token-foreground)]">
            {t('settings.computerUse.title')}
          </h2>
          <label className="flex items-center gap-2 text-sm text-[var(--color-token-text-secondary)] cursor-pointer">
            <input
              type="checkbox"
              checked={computerUseEnabled}
              onChange={e => toggleComputerUseEnabled(e.target.checked)}
              className="rounded border-[var(--color-token-border)] accent-[var(--color-brand)]"
            />
      ...
      {checkState === 'loading' ? (
        <div className="py-8 text-center text-sm text-[var(--color-token-text-secondary)]">
          {t('common.loading')}
        </div>
      ) : checkState === 'error' ? (
        ...
        <>
          ...
          <div className="space-y-2 rounded-lg border border-[var(--color-token-border)] bg-[var(--color-surface-container-low)] p-4">
            <label htmlFor="computer-use-python-path" className="block text-sm font-medium text-[var(--color-token-foreground)]">
            ...
            <div className="flex flex-wrap gap-2">
              <input
                ...
                className="min-w-[220px] flex-1 rounded-lg border border-[var(--color-token-border)] bg-[var(--color-surface-container)] px-3 py-2 font-mono text-xs text-[var(--color-token-foreground)] placeholder:text-[var(--color-token-text-secondary)] focus:border-[var(--color-brand)] focus:outline-none"
              />
              <button
                onClick={choosePythonPath}
                disabled={pythonPathSaving}
                className="flex items-center gap-1.5 rounded-lg border border-[var(--color-token-border)] px-3 py-2 text-xs font-medium text-[var(--color-token-text-secondary)] hover:bg-[var(--color-surface-hover)] disabled:opacity-50"
              >
              ...
              >
                <span className="material-symbols-outlined icon-sm">{pythonPathSaving ? 'hourglass_empty' : 'save'}</span>
            ...
            <>
              ...
              {(accessibilityNeedsAttention || screenRecordingNeedsAttention) && (
                <div className="flex flex-col gap-2 px-4 py-3 rounded-lg bg-yellow-500/5 border border-yellow-500/20">
                  <p className="text-xs text-[var(--color-token-text-secondary)]">{t('settings.computerUse.permRestartHint')}</p>
                  <div className="flex gap-2">
                    {accessibilityNeedsAttention && (
                      <button
                        onClick={() => openSystemSettings('Privacy_Accessibility')}
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-[var(--color-text-accent)] border border-[var(--color-token-border)] rounded-lg hover:bg-[var(--color-surface-hover)]"
                      >
          ...
          {allReady && (status.platform !== 'darwin' || (status.permissions.accessibility && screenRecordingReady)) && (
            <div className="px-4 py-3 rounded-lg bg-green-500/10 border border-green-500/30 text-sm text-green-600 flex items-center gap-2">
              <span className="material-symbols-outlined icon-md" style={{ fontVariationSettings: "'FILL' 1" }}>verified</span>
          ...
          {setupResult && (
            <div className={`rounded-lg border p-4 space-y-2 ${setupResult.success ? 'bg-green-500/5 border-green-500/30' : 'bg-red-500/5 border-red-500/30'}`}>
              ...
              {setupResult.steps.map((step, i) => (
                <div key={i} className="flex items-center gap-2 text-xs text-[var(--color-token-text-secondary)]">
          ...
          <div className="flex gap-3">
            {!status.python.installed && (
              ...
              >
                <span className="material-symbols-outlined icon-md">open_in_new</span>
            ...
            {!envReady && status.python.installed && (
              ...
              >
                <span className="material-symbols-outlined icon-md">{setupRunning ? 'hourglass_empty' : 'download'}</span>
                {setupRunning ? t('settings.computerUse.setupRunning') : t('settings.computerUse.setupBtn')}
              </button>
            )}
            <button
              onClick={fetchStatus}
              className="flex items-center gap-2 px-4 py-2.5 text-sm text-[var(--color-token-text-secondary)] hover:text-[var(--color-token-foreground)] border border-[var(--color-token-border)] rounded-lg hover:bg-[var(--color-surface-hover)] transition-colors"
            >
          ...
          {envReady && (
            <div className="space-y-4 pt-4 border-t border-[var(--color-token-border)]">
              <div>
                <h3 className="text-base font-semibold text-[var(--color-token-foreground)] flex items-center gap-2">
                  {t('settings.computerUse.appsTitle')}
                  {appsSaved && (
                    <span className="text-xs font-normal text-green-500 flex items-center gap-1">
                      <span className="material-symbols-outlined icon-xs" style={{ fontVariationSettings: "'FILL' 1" }}>check</span>
              ...
              <div className="relative">
                <span className="material-symbols-outlined icon-md text-[var(--color-token-text-secondary)] absolute left-3 top-1/2 -translate-y-1/2">search</span>
                <input
                  type="text"
                  value={searchQuery}
                  onChange={e => setSearchQuery(e.target.value)}
                  placeholder={t('settings.computerUse.appsSearch')}
                  className="w-full pl-9 pr-4 py-2 text-sm bg-[var(--color-surface-container-low)] border border-[var(--color-token-border)] rounded-lg text-[var(--color-token-foreground)] placeholder:text-[var(--color-token-text-secondary)] focus:outline-none focus:border-[var(--color-brand)]"
                />
              </div>

              {/* App list */}
              {appsLoading ? (
                <div className="py-6 text-center text-sm text-[var(--color-token-text-secondary)]">
                ...
                <div className="max-h-[400px] overflow-y-auto rounded-lg border border-[var(--color-token-border)]">
                  {sortedApps.map(app => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-4 py-2.5 text-left transition-colors hover:bg-[var(--color-surface-hover)] border-b border-[var(--color-token-border)] last:border-b-0 ${
                      ...
                      >
                        <div className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 border ${
                          ...
                          {isAuthorized && (
                            <span className="material-symbols-outlined icon-xs text-white" style={{ fontVariationSettings: "'FILL' 1" }}>check</span>
                          )}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="text-sm font-medium text-[var(--color-token-foreground)] truncate">
                            {app.displayName}
                          </div>
                          <div className="text-[11px] text-[var(--color-token-text-secondary)] truncate font-mono">
                            {app.bundleId}
                          </div>
                        </div>
                      </button>
