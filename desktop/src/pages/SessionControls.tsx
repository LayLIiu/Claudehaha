    ...
    <div className="h-screen w-screen bg-background text-on-surface font-body selection:bg-primary-fixed overflow-hidden relative">
      ...
      <header className="bg-[var(--color-background)] font-headline font-semibold tracking-wide text-sm fixed top-0 left-0 right-0 flex justify-between items-center px-6 h-12 z-40">
        <div className="flex items-center gap-6">
          <span className="text-sm font-bold text-[var(--color-token-foreground)] uppercase tracking-tighter">
            Claude Code Companion
          </span>
          <nav className="hidden md:flex gap-4">
            <a className="text-[var(--color-token-foreground)] border-b-2 border-[var(--color-brand)] pb-1 cursor-pointer active:opacity-70">
              Code
            </a>
            <a className="text-[var(--color-token-text-secondary)] hover:text-[var(--color-brand)] transition-colors cursor-pointer active:opacity-70">
              Terminal
            </a>
            <a className="text-[var(--color-token-text-secondary)] hover:text-[var(--color-brand)] transition-colors cursor-pointer active:opacity-70">
              History
        ...
        <div className="flex items-center gap-3">
          <span className="material-symbols-outlined text-[var(--color-token-text-secondary)] cursor-pointer">
            arrow_back_ios
          </span>
          <span className="material-symbols-outlined text-[var(--color-token-text-secondary)] cursor-pointer">
            arrow_forward_ios
      ...
      <aside className="bg-[var(--color-surface-container-low)] font-body text-sm font-medium fixed left-0 top-0 h-full w-[280px] hidden md:flex flex-col p-4 gap-2 pt-16 z-30">
        ...
        {/* Nav items */}
        <button className="w-full text-left p-2.5 bg-[var(--color-background)] text-[var(--color-token-foreground)] rounded-lg relative before:content-[''] before:absolute before:left-[-8px] before:w-1 before:h-4 before:bg-[var(--color-brand)] before:rounded-full before:top-1/2 before:-translate-y-1/2 transition-all duration-200 ease-in-out flex items-center gap-3">
          <span className="material-symbols-outlined text-[var(--color-brand)]">add</span>
          <span>New session</span>
        </button>
        <button className="w-full text-left p-2.5 text-[var(--color-token-text-secondary)] hover:bg-[var(--color-surface-hover)] rounded-lg transition-all duration-200 ease-in-out flex items-center gap-3">
          <span className="material-symbols-outlined">calendar_today</span>
          <span>Scheduled</span>
        </button>
        <button className="w-full text-left p-2.5 text-[var(--color-token-text-secondary)] hover:bg-[var(--color-surface-hover)] rounded-lg transition-all duration-200 ease-in-out flex items-center gap-3" data-count={mockSessions.today.length}>
          <span className="material-symbols-outlined">history</span>
          <span>Today</span>
        </button>
        <button className="w-full text-left p-2.5 text-[var(--color-token-text-secondary)] hover:bg-[var(--color-surface-hover)] rounded-lg transition-all duration-200 ease-in-out flex items-center gap-3" data-count={mockSessions.previous7Days.length}>
          <span className="material-symbols-outlined">event_note</span>
          <span>Previous 7 Days</span>
        </button>
        <button className="w-full text-left p-2.5 text-[var(--color-token-text-secondary)] hover:bg-[var(--color-surface-hover)] rounded-lg transition-all duration-200 ease-in-out flex items-center gap-3" data-count={mockSessions.older.length}>
    ...
    return nil
      ...
      <div className="fixed inset-0 z-50 flex flex-col justify-end items-center pointer-events-none p-8">
        ...
        <div className="w-full max-w-2xl flex gap-4 mb-4 pointer-events-auto items-end">
          ...
          {showPermissions && (
            <div
              className="w-80 rounded-xl border border-[var(--color-token-border)] overflow-hidden flex flex-col"
              style={{
            ...
            >
              <div className="px-4 py-3 bg-surface-container-low border-b border-[var(--color-token-border)]">
          ...
          {showModelConfig && (
            <div
              className="w-64 rounded-xl border border-[var(--color-token-border)] overflow-hidden flex flex-col"
              style={{
                background: 'rgba(255, 255, 255, 0.85)',
                backdropFilter: 'blur(20px)',
                boxShadow:
            ...
            >
              ...
              {/* Divider */}
              <div className="mx-4 h-[1px] bg-[var(--color-token-border)]" />
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-surface-container-low border border-[var(--color-token-border)] text-xs font-medium hover:bg-surface-container-high transition-all"
              >
      ...
      {/* ─── Footer / Status Bar ───────────────────────────────── */}
      <footer className="bg-[var(--color-background)] font-body text-xs tracking-tight fixed bottom-0 left-0 w-full h-8 border-t border-[var(--color-token-border)]/20 flex items-center justify-between px-4 z-[60]">
        <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="text-[var(--color-token-text-secondary)] hover:bg-[var(--color-surface-container-low)] transition-colors px-2 py-0.5 rounded">
