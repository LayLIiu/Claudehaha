...
vi.mock('../components/chat/ChatInput', () => ({
  ChatInput: ({
    ...
    <div data-testid="chat-input" data-compact={compact ? 'true' : 'false'} data-variant={variant}>
      <button type="button" onClick={onSubmitStart}>Mock submit start</button>
    </div>
  ),
}))
...
vi.mock('../components/layout/OpenProjectMenu', () => ({
  OpenProjectMenu: ({ path }: { path?: string | null }) => (
    <div data-testid="open-project-menu" data-path={path ?? ''} />
  ),
}))
...
describe('ActiveSession task polling', () => {
  it('新建空白对话时让输入框浮到中间，发送后回落到底部区域', () => {
    const sessionId = 'hero-composer-session'

    useSessionStore.setState({
      sessions: [{
        id: sessionId,
        title: 'Hero Composer Session',
    ...
    useTabStore.setState({
      tabs: [{ sessionId, title: 'Hero Composer Session', type: 'session', status: 'idle' }],
    ...
    expect(screen.getByTestId('chat-input-dock-region')).toHaveClass('chat-input-dock-region--floating')

    act(() => {
      fireEvent.click(screen.getByRole('button', { name: 'Mock submit start' }))
    })

    expect(screen.getByTestId('chat-input')).toHaveAttribute('data-variant', 'default')
    expect(screen.getByTestId('chat-input-dock-region')).not.toHaveClass('chat-input-dock-region--floating')
  })

  it('在桌面聊天标题栏右上角显示 Finder 打开按钮，并保留拖动区域', () => {
    const sessionId = 'session-header-controls'

    useSessionStore.setState({
      sessions: [{
        id: sessionId,
        title: 'Header Controls Session',
        createdAt: '2026-05-07T00:00:00.000Z',
        modifiedAt: '2026-05-07T00:00:00.000Z',
        messageCount: 1,
        projectPath: '/workspace/project',
    ...
    useTabStore.setState({
      tabs: [{ sessionId, title: 'Header Controls Session', type: 'session', status: 'idle' }],
      activeTabId: sessionId,
    })
    useChatStore.setState({
      sessions: {
        [sessionId]: {
          messages: [{ id: 'm1', type: 'assistant_text', content: 'hello', timestamp: 1 }],
          chatState: 'idle',
          connectionState: 'connected',
          streamingText: '',
          streamingToolInput: '',
  ...
  it('keeps the docked terminal mounted when the panel is hidden', async () => {
    ...
    expect(useTerminalPanelStore.getState().isPanelOpen(sessionId)).toBe(false)
    expect(screen.getByTestId('session-terminal-panel').parentElement).toHaveClass('max-h-0', 'opacity-0', 'overflow-hidden')
