    ...
    <div className="max-w-4xl">
      <div className="flex items-start justify-between gap-4 mb-5">
        <div>
          ...
          <p className="text-sm text-[var(--color-token-text-secondary)] mt-0.5">{t('settings.diagnostics.description')}</p>
        </div>
      ...
      <div className="border border-[var(--color-token-border)] rounded-lg mb-5">
        <div className="px-4 py-3 border-b border-[var(--color-token-border)] flex items-center justify-between gap-3">
          <div>
            <div className="text-sm font-medium text-[var(--color-token-foreground)]">{t('settings.diagnostics.logDirectory')}</div>
            <div className="text-xs text-[var(--color-token-text-secondary)] font-mono break-all mt-0.5">{status?.logDir ?? '-'}</div>
          </div>
        ...
        <div className="px-4 py-3 flex flex-wrap items-center gap-2">
          <Button size="sm" onClick={handleExport} loading={isExporting}>
            <span className="material-symbols-outlined icon-sm">archive</span>
          ...
          {lastExportPath && (
            <span className="text-xs text-[var(--color-token-text-secondary)] font-mono break-all">
              {lastExportPath}
            </span>
          )}
        </div>
      ...
      <div className="mb-3">
        <h3 className="text-sm font-semibold text-[var(--color-token-foreground)]">{t('settings.diagnostics.recentEvents')}</h3>
        <p className="text-xs text-[var(--color-token-text-secondary)] mt-0.5">{t('settings.diagnostics.privacyNote')}</p>
      </div>

      <div className="border border-[var(--color-token-border)] rounded-lg overflow-hidden">
        {events.length === 0 ? (
          <div className="px-4 py-8 text-sm text-[var(--color-token-text-secondary)] text-center">
            {isLoading ? t('common.loading') : t('settings.diagnostics.noEvents')}
          </div>
        ) : (
          <div className="divide-y divide-[var(--color-token-border)]">
            {events.map((event) => (
              <EventRow
                key={event.id}
                event={event}
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="border border-[var(--color-token-border)] rounded-lg px-3 py-2">
      <div className="text-xs text-[var(--color-token-text-secondary)]">{label}</div>
      <div className="text-sm font-semibold text-[var(--color-token-foreground)] mt-1">{value}</div>
    </div>
  )
}
...
function EventRow({
  ...
  return (
    <div className="px-4 py-3 grid grid-cols-[120px_92px_1fr] gap-3 items-start">
      <div className="text-xs text-[var(--color-token-text-secondary)] font-mono">
        {new Date(event.timestamp).toLocaleString()}
      </div>
      <div className={`text-xs font-semibold uppercase ${severityClass}`}>{event.severity}</div>
      <div className="min-w-0">
        <div className="flex items-center gap-2 min-w-0">
          <span className="text-sm font-medium text-[var(--color-token-foreground)] truncate">{event.type}</span>
          {event.sessionId && (
            <span className="text-[11px] text-[var(--color-token-text-secondary)] font-mono truncate">{event.sessionId}</span>
          )}
        </div>
        <div className="text-xs text-[var(--color-token-text-secondary)] mt-1 break-words">{event.summary}</div>
        {detailsText && (
          <details className="mt-2">
            <summary className="cursor-pointer text-xs text-[var(--color-token-text-secondary)] select-none">
              {detailsLabel}
            </summary>
            <pre className="mt-2 max-h-64 overflow-auto whitespace-pre-wrap break-words rounded-md border border-[var(--color-token-border)] bg-[var(--color-bg-secondary)] p-2 text-[11px] leading-relaxed text-[var(--color-token-text-secondary)]">
              {detailsText}
            </pre>
          </details>
        )}
