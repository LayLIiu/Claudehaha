        ...
        return Button {
            ...
            HStack(spacing: 10) {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    if let desc = option.trimmedDescription {
                        Text(desc)
                            .font(AppFont.caption())
                            .foregroundStyle(isSelected ? Color.purple.opacity(0.6) : .secondary)
                    }
                }
            ...
            .background(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .fill(isSelected ? Color.purple.opacity(0.08) : Color(.systemBackground).opacity(0.6))
                    .overlay(
                        RoundedRectangle(cornerRadius: 14, style: .continuous)
                            .stroke(isSelected ? Color.purple.opacity(0.3) : Color(.separator).opacity(0.15), lineWidth: 1)
                    )
            )
        }
        .buttonStyle(.plain)
