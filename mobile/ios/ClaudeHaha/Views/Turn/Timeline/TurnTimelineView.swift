...
struct TurnTimelineView<EmptyState: View, Composer: View>: View {
    ...
    // handleScrollPhaseChange 逻辑已内联到 TurnTimelineScrollObserverModifier 中
...
private struct TurnTimelineScrollObserverModifier: ViewModifier {
    ...
    let onTapOutsideComposer: () -> Void
    let onUserDragStarted: () -> Void
    let onUserDragEnded: () -> Void
    ...
    func body(content: Content) -> some View {
        if #available(iOS 18.0, *) {
            content
                ...
                .modifier(ScrollPhaseChangeModifier(onUserDragStarted: onUserDragStarted, onUserDragEnded: onUserDragEnded))
...
// MARK: - iOS 18+ Scroll Phase Change Modifier
        ...
        if #available(iOS 18.0, *) {
            content.onScrollPhaseChange { oldPhase, newPhase in
                ...
                case .tracking, .interacting:
                    onUserDragStarted()
                case .decelerating:
                    if oldPhase == .tracking || oldPhase == .interacting {
                        onUserDragEnded()
                    }
                case .idle:
                    if oldPhase == .tracking || oldPhase == .interacting {
